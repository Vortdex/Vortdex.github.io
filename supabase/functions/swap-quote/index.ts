...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (typeof sellToken !== 'string' || typeof buyToken !== 'string' || typeof sellAmount !== 'string') {
      return new Response(JSON.stringify({ error: 'sellToken, buyToken (string) and sellAmount (string) are required' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!taker || typeof taker !== 'string' || !ETH_ADDRESS_RE.test(taker)) {
      return new Response(JSON.stringify({ error: 'A valid taker address is required for quotes' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    const params = new URLSearchParams({
      chainId: String(chain), sellToken, buyToken, sellAmount, taker,
      swapFeeRecipient: FEE_RECIPIENT, swapFeeBps: FEE_BPS, swapFeeToken: buyToken,
    });
